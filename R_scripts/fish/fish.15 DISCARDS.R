...
pelagic_discards<-read.table("./Data/IMR/Discards/Discards.txt",sep=";",dec=".",header = TRUE)%>% #from reference fleet Tom Clegg unpublished data
  ...
  summarise(Discards=sum(Discards,na.rn=TRUE))%>% #add the discarded biomass of the 2 areas
  ungroup()%>%
...
pelagic_discards<-(pelagic_discards/ICES_area_size)*1e-3

IMR<-IMR_landings/domain_IMR

a<-t(pelagic_discards)/(IMR+t(pelagic_discards))
...
pelagic_discard<-rbindlist(list(Barents1,Barents2))%>% #from reference fleet Tom Clegg unpublished data
...
Discards<-gillnets_fish+pelagic_discard #Create a matrix with all discards
#----------------------------------------------
#Gillnets cetaceans
...
Discards["Longlines_and_Gillnets","Cetacean"]<-porpoise+white_sided
...
Discards["Longlines_and_Gillnets","Pinnipeds"]<-seals
...
Discards["Longlines_and_Gillnets","Birds"]<-birds

Discards<-Discards*1e-3 #let's convert to tonnes

Discards_rate=(Discards)/(t(IMR_landings)+Discards)

Discards_rate[is.na(Discards_rate)]<-0

saveRDS(rate, "./Objects/International discard weight.rds")
