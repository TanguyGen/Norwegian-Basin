...
Files <- list.files(path = "./Data/Fiskeridirektoratet/Offal",
                    pattern = "*.csv",
...
with_progress({
  ...
  Offals <- future_map(Files, ~ {
    ...
    fread(.x, , sep = ";", dec = ",") %>%
      ...
      filter(`Rundvekt` != 0) %>%
      mutate(
        Offal = (`Rundvekt` - `Produktvekt`) / `Rundvekt`,
        Gear_code = `Redskap (kode)`,
        FAO = `Art FAO (kode)`
      ) %>%
      filter(between(`Lat (lokasjon)`, 62, 75),
    ...
    filter(is.na(Aggregated_gear) == FALSE, is.na(Guild) == FALSE) %>%
